<script>
  ...
  function validateForm() {
    ...
    if (!projectData.principalGoal.trim()) errors.principalGoal = t.required;
    if (!projectData.currentProblem.trim()) errors.currentProblem = t.required;
    if (!projectData.targetAudience.trim()) errors.targetAudience = t.required;
...
<div class="max-w-3xl mx-auto">
  ...
  <div class="mb-6 md:mb-8">
    ...
    <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2 sm:mb-3">{$translations.projectDescription?.title || 'Project Overview'}</h2>
    <p class="text-base sm:text-lg text-gray-600">{$translations.projectDescription?.subtitle || 'Help us understand...'}</p>
  </div>
    ...
    <form on:submit|preventDefault={handleNext} class="space-y-4 md:space-y-6">
      ...
      <div class="bg-gradient-to-r from-blue-50 to-transparent p-4 sm:p-6 rounded-xl border border-blue-100">
        <div class="flex items-start gap-3 sm:gap-4">
          <div class="hidden sm:flex w-10 h-10 bg-blue-600 rounded-lg items-center justify-center flex-shrink-0"><svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
          <div class="flex-1">
            <TextArea
              label={$translations.projectDescription?.principalGoal}
              bind:value={projectData.principalGoal}
              placeholder={$translations.projectDescription?.principalGoalPlaceholder}
              error={errors.principalGoal}
              required={true}
              rows={3}
              helpText={$translations.projectDescription?.principalGoalHelpText}
            />
          </div>
        </div>
      </div>
      ...
      <div class="bg-gradient-to-r from-yellow-50 to-transparent p-4 sm:p-6 rounded-xl border border-yellow-100">
        <div class="flex items-start gap-3 sm:gap-4">
          <div class="hidden sm:flex w-10 h-10 bg-yellow-500 rounded-lg items-center justify-center flex-shrink-0"><svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg></div>
          <div class="flex-1">
            <TextArea
              label={$translations.projectDescription?.currentProblem}
              bind:value={projectData.currentProblem}
              placeholder={$translations.projectDescription?.currentProblemPlaceholder}
              error={errors.currentProblem}
              required={true}
              rows={3}
              helpText={$translations.projectDescription?.currentProblemHelpText}
            />
          </div>
        </div>
      </div>
      ...
      <div class="bg-gradient-to-r from-purple-50 to-transparent p-4 sm:p-6 rounded-xl border border-purple-100">
        <div class="flex items-start gap-3 sm:gap-4">
          <div class="hidden sm:flex w-10 h-10 bg-purple-600 rounded-lg items-center justify-center flex-shrink-0"><svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg></div>
          <div class="flex-1">
            <TextArea
              label={$translations.projectDescription?.targetAudience}
              bind:value={projectData.targetAudience}
              placeholder={$translations.projectDescription?.targetAudiencePlaceholder}
              error={errors.targetAudience}
              required={true}
              rows={3}
              helpText={$translations.projectDescription?.targetAudienceHelpText}
            />
          </div>
        </div>
      </div>
